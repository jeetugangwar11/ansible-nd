...
class ProposeVlanQueryParams(EndpointQueryParams):
    ...
    vlan_type: Optional[str] = Field(
        default=None, description="The type of VLAN to query (networkVlan, vrfVlan, serviceNetworkVlan, vpcPeerLinkVlan)"
    )
...
class SwitchesInventoryQueryParams(EndpointQueryParams):
    """
    Endpoint-specific query parameters for the inventory switches endpoint.

    Parameters
    - fabric_name: Fabric name to filter switches (optional)
    """

    fabric_name: Optional[str] = Field(
        default=None, min_length=1, description="Name of the fabric to filter switches"
    )


# =============================================================================
# SWITCHES INVENTORY ENDPOINTS
# =============================================================================


class V1ManageSwitchesGet(BaseModel):
    """
    ND Manage Inventory Switches GET Endpoint

    Path: /api/v1/manage/inventory/switches?fabricName={fabricName}
    Verb: GET
    """

    model_config = COMMON_CONFIG

    endpoint_params: SwitchesInventoryQueryParams = Field(
        default_factory=SwitchesInventoryQueryParams,
